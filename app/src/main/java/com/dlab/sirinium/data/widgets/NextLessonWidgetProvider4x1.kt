...
/**
 * AppWidgetProvider для виджета 3x1 (ранее 4x1).
 * Отображает только следующую пару: дата, название, время и аудитория.
 */
class NextLessonWidgetProvider4x1 : AppWidgetProvider() {
    ...
    private fun initializeDependencies(context: Context): Boolean {
        try {
            ...
            Log.e(TAG_WIDGET_4X1, "Error initializing dependencies: ${e.message}", e)
            return false
        }
    }
    ...
    override fun onUpdate(
        ...
        if (!initializeDependencies(context)) {
            appWidgetIds.forEach { appWidgetId ->
                Log.e(TAG_WIDGET_4X1, "Failed to initialize dependencies for widget ID $appWidgetId during onUpdate.")
                updateWidgetUi(
                    context = context,
                    appWidgetManager = appWidgetManager,
        ...
        appWidgetIds.forEach { appWidgetId ->
            Log.d(TAG_WIDGET_4X1, "onUpdate for widget ID: $appWidgetId")
            val pendingResult = try {
                goAsync()
            } catch (e: Exception) {
                Log.w(TAG_WIDGET_4X1, "goAsync() failed or not available: ${e.message}")
                null
            }
            widgetScope.launch {
                ...
                try {
                    ...
                    if (groupSuffix.isBlank()) {
                        ...
                        Log.d(TAG_WIDGET_4X1, "Fetching for ${if (isTeacher) "teacher" else "group"} $currentGroupForLog, widget ID $appWidgetId")
                                ...
                                when (awaited) {
                                    is NetworkResult.Success -> {
                                        ...
                                        if (awaited.isStale == true) {
                                            Log.w(TAG_WIDGET_4X1, "Data for group $currentGroupForLog, week $weekOffset is stale.")
                                        }
                                    }
                                    is NetworkResult.Error -> {
                                        Log.e(TAG_WIDGET_4X1, "Error fetching for $currentGroupForLog, week $weekOffset: ${awaited.message} (Code: ${awaited.code})")
                                        if (firstErrorFromRepo == null) {
                                            firstErrorFromRepo = awaited.message ?: context.getString(R.string.widget_network_error)
                                        }
                                    }
                                    is NetworkResult.Loading -> { /* Отфильтровано */ }
                                }
                            } catch (e: TimeoutCancellationException) {
                                Log.e(TAG_WIDGET_4X1, "Timeout fetching week $weekOffset for $currentGroupForLog, widget $appWidgetId", e)
                                if (firstErrorFromRepo == null) {
                                    firstErrorFromRepo = context.getString(R.string.widget_network_error)
                                }
                            } catch (e: Exception) {
                                Log.e(TAG_WIDGET_4X1, "Exception fetching week $weekOffset for $currentGroupForLog, widget $appWidgetId", e)
                                if (firstErrorFromRepo == null) {
                                    firstErrorFromRepo = context.getString(R.string.widget_network_error)
                                }
                            }
                                    ...
                                    .mapNotNull { item ->
                                        try {
                                            if (item.date == null || item.startTime == null || item.endTime == null) {
                                                Log.w(TAG_WIDGET_4X1, "ScheduleItem has null date/time: ${item.discipline}")
                                            ...
                                            Log.e(TAG_WIDGET_4X1, "DateTimeParseException for item '${item.discipline}' on date '${item.date}': ${e.message}")
                                            null
                                        } catch (e: Exception) {
                                            Log.e(TAG_WIDGET_4X1, "Error mapping ScheduleItem '${item.discipline}': ${e.message}")
                                            null
                                        }
                                    }
                                    .sortedBy { it.first }
                    ...
                    Log.e(TAG_WIDGET_4X1, "General error during onUpdate for widget $appWidgetId, group ${currentGroupForLog ?: "N/A"}", e)
                    ...
                    try {
                        ...
                        Log.w(TAG_WIDGET_4X1, "pendingResult.finish() failed: ${e.message}")
                    }
                    Log.d(TAG_WIDGET_4X1, "Finished async work for widget ID: $appWidgetId. Final Message: $displayMessage")
                }
            }
        }
    }
    ...
    private fun updateWidgetUi(
        ...
        Log.d(TAG_WIDGET_4X1, "updateWidgetUi for ID $appWidgetId. Next: ${nextLesson?.discipline}, Error: $errorMessage, Theme: $themeSetting")

        val views = RemoteViews(context.packageName, R.layout.widget_next_lesson_4x1)
        ...
        if (nextLesson != null) {
            ...
            val monthText = dateOfLesson.month.getDisplayName(java.time.format.TextStyle.FULL_STANDALONE, Locale("ru"))
            views.setTextViewText(R.id.widget_tv_date_day_4x1, "$dayOfWeekText, ${dateOfLesson.dayOfMonth} $monthText")
            views.setTextViewText(R.id.widget_tv_lesson_name_4x1, nextLesson.discipline ?: "")
            views.setTextViewText(R.id.widget_tv_lesson_time_4x1, "${nextLesson.startTime} - ${nextLesson.endTime}")
            ...
            if (classroomText.isNotBlank()) {
                ...
                views.setInt(R.id.widget_classroom_container_4x1, "setBackgroundResource",
            ...
            views.setInt(R.id.widget_tv_classroom_4x1, "setTextColor", context.getColor(android.R.color.white))
        } else {
            ...
            views.setInt(R.id.widget_tv_message_4x1, "setTextColor", messageTextColor)
        }
    ...
    override fun onDisabled(context: Context) {
        ...
        if (widgetScope.isActive) {
            widgetScope.cancel()
            Log.d(TAG_WIDGET_4X1, "Last widget disabled, widgetScope cancelled.")
        }
    }
}
